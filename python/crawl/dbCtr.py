...
class MysqlCtr(object):
    ...
    def insertData(self, tb, info):
        sql = "INSERT INTO " + tb + "(name, link)VALUES(%s, %s)"
        try:
            self.cursor.execute(sql, [info['name'], info['link']])
            self.conn.commit()
    ...
    def createTb(self, tb):
        sql = "CREATE TABLE IF NOT EXISTS " + tb + "(name VARCHAR(50) NOT NULL,link TEXT)"
    ...
    def updateData(self, tb, name, info):
        sql = "UPDATE " + tb + " SET link=%s where name=%s"
        try:
            self.cursor.execute(sql, [info, name])
            self.conn.commit()
        except:
